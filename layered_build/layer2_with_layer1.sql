...
SELECT
  ...
  -- OH = 0 AND no PO (most critical)
  CASE
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) = 0
  ...
  CASE
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) > 0
         AND COALESCE(l2.site_oh_qty, ls.site_oh_qty) <= COALESCE(l2.min_level, ls.min_level)
         AND COALESCE(l2.back_order_qty, lco.back_order_qty, 0.0) = 0 THEN 1
    ELSE 0
  ...
  -- NULL when there is no stock record for this site+part.
  ...
  --   0.00  on-hand > MIN
  CASE
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) IS NULL THEN NULL
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) = 0
         AND COALESCE(l2.back_order_qty, lco.back_order_qty, 0.0) = 0 THEN 1.00
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) = 0 THEN 0.75
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) <= COALESCE(l2.min_level, ls.min_level)
         AND COALESCE(l2.back_order_qty, lco.back_order_qty, 0.0) = 0 THEN 0.50
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) <= COALESCE(l2.min_level, ls.min_level) THEN 0.25
    ELSE 0.00
  END AS situational_score_150d,

  -- Situational score criticality = sto_class weight × situational tier.
  -- NULL when no stock record exists.
  CASE WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) IS NULL THEN NULL ELSE
    ROUND(
  ...
  CASE
    WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) IS NULL
         AND l2.structural_risk_combo_criticality_150d IS NULL THEN NULL
    ELSE GREATEST(0.0, ROUND(
      COALESCE(
        (CASE COALESCE(l2.sto_class, ls.sto_class)
         ...
         END)
        * CASE
            WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) IS NULL THEN 0.0
            WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) = 0
                 AND COALESCE(l2.back_order_qty, lco.back_order_qty, 0.0) = 0 THEN 1.00
            WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) = 0 THEN 0.75
            WHEN COALESCE(l2.site_oh_qty, ls.site_oh_qty) <= COALESCE(l2.min_level, ls.min_level)
