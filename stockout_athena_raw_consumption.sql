-- stockout_athena_raw_consumption.sql
-- Stockout Score Calculation — ALL parts from hw_raw_consumption_daily
-- Consumption source: hw_raw_consumption_daily (qty_consumed)
-- Order history: DWEEB tables for AR parts, r5orderlines for non-AR parts
-- Source label: 'AR' if part exists in dweeb_hx_order, 'not AR' otherwise

WITH
-- Target parts from hw_raw_consumption_daily
target_parts AS (
...
-- Part description + cat_ref from catalogue (one row per sto_part + cat_ref)
part_info AS (
  SELECT sto_part, part_description, cat_ref, region
  FROM (
    SELECT cat_part AS sto_part, cat_desc AS part_description, cat_ref, region,
           ROW_NUMBER() OVER (PARTITION BY cat_part, cat_ref ORDER BY LENGTH(cat_desc) DESC) AS rn
    FROM (
...
metrics AS (
  SELECT s.site, s.region, s.sto_part,
         ...
         pi.part_description, pi.cat_ref,
         -- Derive part_number from cat_ref (for DWEEB join)
         CASE
           WHEN UPPER(TRIM(pi.cat_ref)) LIKE 'R%' OR UPPER(TRIM(pi.cat_ref)) LIKE '%-FRU'
           THEN REPLACE(REPLACE(UPPER(TRIM(pi.cat_ref)), '-FRU', ''), 'R', '')
           ELSE UPPER(TRIM(pi.cat_ref))
         END AS part_number,
         bt.building_type,
...
SELECT
  ...
  m.site, m.region, m.sto_part AS part, m.amazon_pn, m.part_description, m.cat_ref, m.part_number,
...
WHERE COALESCE(m.consumed_365d, 0) > 0 AND m.sto_class IN ('01 HIGH', '02 MED', '03 LOW')
ORDER BY m.site, structural_risk_combo_criticality_150d DESC NULLS LAST, m.sto_part;
