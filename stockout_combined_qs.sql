...
r5stock_site_counts AS (
  ...
  FROM (
    ...
    FROM andes_bi_ext."rme-gdl".r5stock_apm_na
    UNION ALL
    SELECT SPLIT_PART(sto_store, '-', 1) AS site, sto_part
    FROM andes_bi_ext."rme-gdl".r5stock_apm_eu
  ) all_stock
  GROUP BY site
)
...
FROM deduped c
  LEFT JOIN andes_bi_ext."ar-performance-n-insights".rts_rcc_facilities fac ON fac.code = c.site
