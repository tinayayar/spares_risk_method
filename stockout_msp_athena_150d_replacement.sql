-- stockout_msp_athena_150d_replacement.sql
...
-- Stock levels: min/max/current per site (deduplicated — prefer NA over EU for cross-listed sites)
stock AS (
...
-- Replacements from hw_part_family_site_equipment_consumption
replacements AS (
  SELECT site,
         region,
         apn AS amzn_part,
...
metrics AS (
  ...
  FROM stock s
    LEFT JOIN apn_product_model apm ON apm.apn = s.sto_part AND apm.site = s.site
